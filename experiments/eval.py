...
judges = ['Llama-3.2-3B-Instruct', 'Mistral-7B-Instruct-v0.3',
        'gemma-3-4b-it', 'DeepSeek-R1-Distill-Llama-8B']
...
def evaluate(file_name: str = "", test = "2"):
    assistants = ['Llama-3.2-3B-Instruct', 
                'Mistral-7B-Instruct-v0.3',
                'gemma-3-4b-it',
                'gemma-3-12b-it',
                'DeepSeek-R1-Distill-Llama-8B']
    fail_count = 0
    record = {task: {judge: {assist: {} for assist in assistants}
                    for judge in judges} for task in range(1, 5)}
    fails = {judge: 0 for judge in judges}
    if file_name:
        fp = os.path.join(
            script_dir, "..", "results", "judgement", test, file_name)
        with open(fp, "r") as f:
            data = json.load(f)
            for instance in data:
                ...
                if not rating:
                    ...
                    record[task][judge][assistant][word] = (
                        record[task][judge][assistant][word][0]+int(rating)/10, record[task][judge][assistant][word][1]+1)
    else:
        result_path = os.path.join(
            script_dir, "..", "results", "judgement", test)
                ...
                with open(fp, "r") as f:
                    ...
                    for instance in data:
                        ...
                        if not rating:
                            ...
                            record[task][judge][assistant][word] = (
                                record[task][judge][assistant][word][0]+1, record[task][judge][assistant][word][1]+int(rating)/10)
...
def plot_1(results, assistants=['Llama-3.2-3B-Instruct',
                                'Mistral-7B-Instruct-v0.3',
                                'gemma-3-4b-it', 'DeepSeek-R1-Distill-Llama-8B']):
    palette = sns.color_palette("tab10", len(assistants))
    color_map = dict(zip(assistants, palette))
    slopes = {task: {judge: {assist: {"m": 0, "b": 0} for assist in assistants}
                    for judge in judges} for task in range(1, 5)}
    ...
    for row, task in enumerate(results):
        ...
        for col, judge in enumerate(judges):
            ...
            for assistant in assistants:
                ax = axs[row][col]
                x = [score[0]
                    for score in results[task][judge][assistant].values()]
                y = [score[1]
                    for score in results[task][judge][assistant].values()]
                if x and y:
                    limit = max(x+y)
                    ...
                    ax.scatter(
                        x, y, color=color_map[assistant], s=1, label='-'.join(assistant.split('-')[:3]).capitalize())
    ...
    for row in range(len(results)):
        for col in range(len(judges)):
            ...
            all_handles_labels.extend([(l, h)
                                    for h, l in zip(handles, labels)])
    all_handles_labels = dict(all_handles_labels)
    fig.legend([handle for handle in all_handles_labels.values()], [
                label for label in all_handles_labels], ncol=3, loc='upper center', fontsize=12, markerscale=10)
    plt.show()
    return slopes


def plot_2(results, assistants=['Llama-3.2-3B-Instruct',
                                'Mistral-7B-Instruct-v0.3',
                                'gemma-3-4b-it', 'DeepSeek-R1-Distill-Llama-8B'],
                                tasks=[2, 3, 4]):
            ...
            for col, judge in enumerate(judges):
                ...
                for row, assistant in enumerate(assistants):
                    ...
                    if x and y:
                        ...
                        ax.plot([0, limit], [0, limit], '--',
                                color='gray', linewidth=1, zorder=0)
                        ax.scatter(x, y, color=color_map[task], s=1, label=f"Task {task}" if (
                            row == 0 and col == 0) else "")
                        m, b = np.polyfit(x, y, 1)
...
if __name__ == "__main__":
    ...
    assistant_lines.update(plot_1(results, assistants=['gemma-3-4b-it', 'gemma-3-12b-it']))
