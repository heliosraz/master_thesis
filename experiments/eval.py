...
assistants = ['Llama-3.2-3B-Instruct', 'Mistral-7B-Instruct-v0.3',
              'gemma-3-4b-it', 'gemma-3-12b-it', 'DeepSeek-R1-Distill-Llama-8B']
...
def plot_1(results):
    ...
    fig.legend([handle for handle in all_handles_labels.values()], [label for label in all_handles_labels], ncol = 3, loc='upper center', fontsize=12, markerscale=10)
    plt.show()
    return slopes
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        results = evaluate()
    print(plot_1(results))
    print(plot_2(results))
