...
class DeepSeek(nn.Module):
    def __init__(self, model_id: str = "deepseek-ai/DeepSeek-R1-Distill-Llama-8B", device: str = "cuda", system_prompt: str = ""):
        super(DeepSeek, self).__init__()
    ...
    def forward(self, prompts: List[List[str]], use_tqdm = False):
        ...
        prompt = [self.system_prompt+"\n" for _ in prompts] if self.system_prompt else ["" for _ in prompts]
        with torch.no_grad():
