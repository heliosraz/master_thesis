...
def format_task(example: DataInstance, task: int):
    if task == 1:
        ...
        prompt = [(f"Question: What words in \"{example.definition}\" are the key words in defining \"{example.word}\""), (
...
def generate_examples(word: str, pairs: Iterable[Tuple[str, str]], task: int, gold: Dict[tuple, str] = None):
    ...
    for defn, sentence in pairs:
        ...
        if defn and filter_sentence:
            ...
            if gold:
                examples.append({'word': word, 'definition': defn,
                                'gold': gold[sentence], 'sentence': sentence[0], 'prompt': prompt})
            else:
                examples.append({'word': word, 'definition': defn,
                                'sentence': sentence[0], 'prompt': prompt})
    return examples
...
def task_helper(word: str, task: int):
    ...
    # Task 1:
    if task == 1:
        instances += generate_examples(word, itertools.product(defns,
                                       sentences), task, gold=sentences)
    # Task 2:
    elif task == 2:
        instances += generate_examples(word,
                                       zip(defns, sentences), task, gold=sentences)
    # Task 3:
    elif task == 3:
